...
jQuery.sap.declare("com.ordina.parkconnectHackaton_ParkConnect.MyRouter");

sap.ui.core.routing.Router.extend("com.ordina.parkconnectHackaton_ParkConnect.MyRouter", {
	 ...
	 * @public
	 */
	myNavTo: function(oOptions) {
		var oSplitApp = this._findSplitApp(oOptions.currentView);
		// Load view, add it to the page aggregation, and navigate to it
		var oView = this.getView(oOptions.targetViewName, oOptions.targetViewType);
		oSplitApp.addPage(oView, oOptions.isMaster);
		oSplitApp.to(oView.getId(), oOptions.data);
		oView.setBindingContext(oOptions.data);
	},
	
	myNavBackWithHash: function(oOptions) {
		var oSplitApp = this._findSplitApp(oOptions.currentView);
	
		// oSplitApp.backToPage(oOptions.targetViewName);
		oSplitApp.backToTopDetail();
	},
